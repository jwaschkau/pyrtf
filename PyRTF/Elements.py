...
class Section( list ) :
    ...
    def __init__(self, paper=None, margins=None, break_type=None, headery=None, footery=None, landscape=None,
    ...
    def __init__(self, *params):
        ...
        for param in params :
            if isinstance(param, TextStyle):
                self.Style = param
            elif isinstance(param, TextPS):
                self.Properties = param
            elif isinstance(param, ShadingPS):
                self.Shading = param
            else:
...
class Paragraph(list):
    def __init__(self, *params):
        ...
        for param in params :
            if isinstance(param, ParagraphStyle):
                self.Style = param
            elif isinstance(param, ParagraphPS):
                self.Properties = param
            elif isinstance(param, FramePS):
                self.Frame = param
            elif isinstance(param, ShadingPS):
                self.Shading = param
            else:
...
class Cell(list):
    ...
    def __init__(self, *params, **kwargs):
        ...
        for param in params :
            if isinstance(param, str):
                self.append(param)
            elif isinstance(param, Paragraph):
                self.append(param)
            elif isinstance(param, FramePS):
                self.SetFrame(param)
            elif isinstance(param, MarginsPS):
                self.SetMargins(param)
    ...
    def SetAlignment(self, value):
        assert value in [self.ALIGN_TOP, self.ALIGN_CENTER,
                         self.ALIGN_BOTTOM]  # , self.ALIGN_TEXT_TOP_DOWN, self.ALIGN_TEXT_BOTTOM_UP ]
        self.Alignment = value

    def SetFlow(self, value):
        assert value in [self.FLOW_LR_TB, self.FLOW_RL_TB, self.FLOW_LR_BT, self.FLOW_VERTICAL_LR_TB,
                         self.FLOW_VERTICAL_TB_RL]
        self.Flow = value
